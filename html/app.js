...
function buildCategoryFilters() {
  ...
  container.innerHTML = categories.map(cat => {
    ...
    return `
      <div class="filter-item ${cat === selectedCategory ? 'active' : ''}" onclick="selectCategory('${cat}')">
        <span>${cat}</span>
...
function buildSubtopicFilters() {
  ...
  container.innerHTML = subtopics.map(sub => {
    ...
    return `
      <div class="filter-item ${sub === selectedSubtopic ? 'active' : ''}" onclick="selectSubtopic('${sub}')">
        <span>${sub}</span>
        <span class="count-badge">${count}</span>
      </div>
    `;
...
function applyFilters() {
  ...
  document.getElementById('paletteCountBadge').innerText = `${filteredQuestions.length} Qs`;
  renderQuestionPalette();
  if (filteredQuestions.length > 0) {
    ...
    document.getElementById('passageBanner').style.display = 'none';
  }
}
...
// Load Question onto Focus Card
function loadQuestion(index) {
  if (index < 0 || index >= filteredQuestions.length) return;
  currentIndex = index;
  const q = filteredQuestions[currentIndex];

  // Update Header Bar Counters
  document.getElementById('currentQIndexText').innerText = currentIndex + 1;
  document.getElementById('totalQCountText').innerText = filteredQuestions.length;
  
  const progressPct = ((currentIndex + 1) / filteredQuestions.length) * 100;
  ...
  const passageBanner = document.getElementById('passageBanner');
...
function selectOption(qId, key) {
  ...
  optionPills.forEach(pill => {
    pill.classList.remove('selected', 'correct', 'incorrect');
...
function calculateAccuracy() {
  ...
  if (totalAnswered === 0) {
    document.getElementById('accuracyText').innerText = '0%';
    return;
  }
  document.getElementById('accuracyText').innerText = `${Math.round((totalAnswered / filteredQuestions.length) * 100)}% Answered`;
}
